-- ============================================================================
-- pop9 — Parte 2: estruturas de restaurante, com prefixo pop9_
-- ============================================================================
--  Roda depois da Parte 1. Todo objeto criado começa com `pop9_` — nada aqui
--  altera, lê ou apaga tabela de produção do fast_bar.
-- ============================================================================
...
create table if not exists public.pop9_fastbar_unidades (
  ...
  constraint pop9_fastbar_unidades_dimensao_check
    check (dimensao in ('massa', 'volume', 'contagem')),
  constraint pop9_fastbar_unidades_fator_positivo check (fator_base > 0)
);

insert into public.pop9_fastbar_unidades (codigo, nome, dimensao, fator_base) values
  ('g',  'grama',      'massa',    1),
...
create or replace function public.pop9_fastbar_converter(
...
begin
  ...
  from public.pop9_fastbar_unidades where id = p_de;
  select dimensao, fator_base into v_para
  from public.pop9_fastbar_unidades where id = p_para;
...
create table if not exists public.pop9_fastbar_itens (
  id uuid primary key default gen_random_uuid(),
  nome text not null,
  tipo text not null,
  unidade_estoque_id uuid not null references public.pop9_fastbar_unidades(id),
  ...
  constraint pop9_fastbar_itens_tipo_check
    check (tipo in ('insumo', 'semiacabado', 'acabado', 'revenda')),
  constraint pop9_fastbar_itens_estoque_nao_negativo check (estoque_atual >= 0)
);

create unique index if not exists pop9_fastbar_itens_nome_ci_key
  on public.pop9_fastbar_itens (lower(nome));
create index if not exists pop9_fastbar_itens_tipo_idx
  on public.pop9_fastbar_itens (tipo) where ativo;
...
create table if not exists public.pop9_fastbar_item_embalagens (
  id uuid primary key default gen_random_uuid(),
  item_id uuid not null references public.pop9_fastbar_itens(id) on delete cascade,
  nome text not null,
  quantidade_por_embalagem numeric not null,
  unidade_conteudo_id uuid not null references public.pop9_fastbar_unidades(id),
  padrao boolean not null default false,
  ativo boolean not null default true,
  created_at timestamptz not null default now(),
  constraint pop9_fastbar_item_embalagens_quantidade_positiva
    check (quantidade_por_embalagem > 0)
);

create unique index if not exists pop9_fastbar_item_embalagens_padrao_key
  on public.pop9_fastbar_item_embalagens (item_id) where padrao;

-- -------------------------------------------------------------- ficha técnica
-- A diferença central para pop9_fastbar_recipe_items: a ficha tem item de SAÍDA e
...
create table if not exists public.pop9_fastbar_fichas (
  id uuid primary key default gen_random_uuid(),
  item_produzido_id uuid not null references public.pop9_fastbar_itens(id) on delete cascade,
  rendimento_quantidade numeric not null,
  rendimento_unidade_id uuid not null references public.pop9_fastbar_unidades(id),
  modo_preparo text,
  versao integer not null default 1,
  ativa boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint pop9_fastbar_fichas_rendimento_positivo check (rendimento_quantidade > 0)
);

-- Uma ficha ativa por item; versões antigas ficam guardadas para histórico de custo.
create unique index if not exists pop9_fastbar_fichas_item_ativa_key
  on public.pop9_fastbar_fichas (item_produzido_id) where ativa;

create table if not exists public.pop9_fastbar_ficha_componentes (
  id uuid primary key default gen_random_uuid(),
  ficha_id uuid not null references public.pop9_fastbar_fichas(id) on delete cascade,
  item_id uuid not null references public.pop9_fastbar_itens(id),
  quantidade numeric not null,
  unidade_id uuid not null references public.pop9_fastbar_unidades(id),
  constraint pop9_fastbar_ficha_componentes_quantidade_positiva check (quantidade > 0),
  constraint pop9_fastbar_ficha_componentes_unica unique (ficha_id, item_id)
);

create index if not exists pop9_fastbar_ficha_componentes_item_idx
  on public.pop9_fastbar_ficha_componentes (item_id);
...
create table if not exists public.pop9_fastbar_porcoes (
  id uuid primary key default gen_random_uuid(),
  item_id uuid not null references public.pop9_fastbar_itens(id) on delete cascade,
  nome text not null,
  quantidade numeric not null,
  ...
  ativo boolean not null default true,
  created_at timestamptz not null default now(),
  constraint pop9_fastbar_porcoes_quantidade_positiva check (quantidade > 0),
  constraint pop9_fastbar_porcoes_unica unique (item_id, nome)
);
...
create table if not exists public.pop9_fastbar_producoes (
  id uuid primary key default gen_random_uuid(),
  ficha_id uuid not null references public.pop9_fastbar_fichas(id),
  item_produzido_id uuid not null references public.pop9_fastbar_itens(id),
  quantidade_produzida numeric not null,
  unidade_id uuid not null references public.pop9_fastbar_unidades(id),
  custo_total numeric not null default 0,
  observacao text,
  created_at timestamptz not null default now(),
  constraint pop9_fastbar_producoes_quantidade_positiva check (quantidade_produzida > 0)
);
...
create table if not exists public.pop9_fastbar_item_movimentos (
  id uuid primary key default gen_random_uuid(),
  item_id uuid not null references public.pop9_fastbar_itens(id) on delete cascade,
  tipo text not null,
  quantidade numeric not null,
  ...
  producao_id uuid references public.pop9_fastbar_producoes(id) on delete set null,
  session_id uuid references public.pop9_fastbar_sessions(id) on delete set null,
  nota text,
  created_at timestamptz not null default now(),
  constraint pop9_fastbar_item_movimentos_tipo_check
    check (tipo in ('entrada', 'saida', 'producao', 'consumo', 'perda', 'ajuste')),
  constraint pop9_fastbar_item_movimentos_quantidade_positiva check (quantidade > 0)
);

create index if not exists pop9_fastbar_item_movimentos_item_idx
  on public.pop9_fastbar_item_movimentos (item_id, created_at desc);
...
begin
  foreach t in array array[
    'pop9_fastbar_unidades', 'pop9_fastbar_itens', 'pop9_fastbar_item_embalagens',
    'pop9_fastbar_fichas', 'pop9_fastbar_ficha_componentes', 'pop9_fastbar_porcoes',
    'pop9_fastbar_producoes', 'pop9_fastbar_item_movimentos'
  ]
  loop
